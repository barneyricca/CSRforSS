...
to setup
  ...
  if initial-police-density + initial-agent-density > 100 [
    user-message (word
      "The sum of INITIAL-POLICE-DENSITY and INITIAL-AGENT-DENSITY "
      "should not be greater than 100.")
    stop
  ]

  ; create officers
  create-officers round (initial-police-density * .01 * count patches) [
...
to go
  ask turtles [
    ...
    if (breed = agents and jail-term = 0) or breed = officers [ move ]
...
to-report estimated-arrest-probability
  let c count officers-on neighborhood
...
; VISUALIZATION OF AGENTS AND POLICES
...
; See Info tab for full policeyright and license.
;
; Modified by BPR
;  "cops" replaced by "officers" (per advice from Barb Rockell. The term is somewhat perjorative)
;  "cop" replaced by "police" (per advice from Barb Rockell)
@#$#@#$#@
GRAPHICS-WINDOW
325
10
